...
# Why are they these values? That's easy, it's because
# they are the means and standard deviations of the 
# RGB channels of the ImageNet dataset.
mean = np.array([0.485, 0.456, 0.406])
std = np.array([0.229, 0.224, 0.225])

data_transforms = {
    'train': transforms.Compose([
        # Here we resize the images to 224x224 pixels 
        transforms.Resize((224,224)),
        transforms.ToTensor(),
        # Here we normalize the images using the pre-computed mean and standard deviation. 
...
# I guess that the directory that their data is in is called "data"?
data_dir = "data"

# A dictionary containing their training and validation datasets
...
def imshow(inp, title):
    '''
    The imshow function takes an input tensor representing an image,
    reverses the normalization, and displays it using matplotlib.

    Parameters:
    -  inp (torch.Tensor): An image tensor to be displayed. 
...
def plot(val_loss,train_loss,typ):
    '''
    The plot function plots and saves the training and validation
    losses of the model.

    Parameters:
    - loss (list): List containing validation loss or accuracy at each epoch.
    - train_loss (list): List containing training loss or accuracy at each epoch.
    - typ (str): Type of the plot, should be "Loss" or "Accuracy".
    '''
...
def train_model(model, criterion, optimizer, scheduler, num_epochs=25,model_name = "kaggle"):
    '''
    The train_model function trains a deep learning model

    Parameters:
    - model (torch.nn.Module): The neural network model to be trained.
    - criterion (torch.nn.Module): The loss function.
    - optimizer (torch.optim.Optimizer): The optimizer for updating model parameters.
    - scheduler (torch.optim.lr_scheduler._LRScheduler): Scheduler to adjust the learning rate.
